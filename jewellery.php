                ...
                while($row = mysqli_fetch_assoc($result)){
                    ...
                    <div class="col-sm" style="padding:50px;">
                        <form class="validation" novalidate
                            action="jewellery.php?txtSearch=<?php echo $search ?>&action=add&id=<?php echo $row['p_id'];?>"
                            method="post">
                            <img class="product" src="assets/images/<?php echo $row['image'] ?>" width=200 height=200 /><input
                                type='hidden' name='image' value="<?php echo $row['image'];?>" />
                            <h5 style="padding-top:20px;"><b><?php echo $row['p_name'];?></b><input type='hidden' name='txtName'
                                    value="<?php echo $row['p_name'];?>" /></h5>
                            ...
                            <?php
                                if($row['discount']>0){
                                ?>
                                    <div><strike>Rs.<?php echo $row['price'];?></strike><input type='hidden' name='txtPrice'
                                            value="<?php echo $row['price'];?>" /></div>
                                    <div style="padding-bottom:20px; color:red;">
                                        <h4>Rs. <?php
                                                        echo $row['sale_price'];?></h4><input type='hidden' name='txtSalePrice'
                                            value="<?php echo $row['sale_price'];?>" />
                                    </div>
                                    <?php
                                }else{
                                ?>
                                    <div style="padding-bottom:20px; padding-top:24px;">
                                        <h4>Rs. <?php
                                                        echo $row['sale_price'];?></h4><input type='hidden' name='txtSalePrice'
                                            value="<?php echo $row['sale_price'];?>" />
                                    </div>
